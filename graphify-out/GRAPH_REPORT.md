...
## Corpus Check
- 50 files · ~14,602 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 501 nodes · 658 edges · 65 communities (41 shown, 24 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 144 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e0138eda`
...
Cohesion: 0.14
Nodes (18): Architecture, Clearing the cache, code:bash (python main.py -config=config/backtest.yaml -strategy=strate), code:yaml (start_date: "2022-01-01"), code:bash (python main.py -config=config/backtest.yaml -strategy=strate), code:python (def run(ctx):), code:block5 (actions/        # one file per action; mock + OpenBB data pr), code:bash (pip install -r requirements.txt) (+10 more)

### Community 6 - "Advance Tests"
Cohesion: 0.13
Nodes (14): Beta, Classification, code:yaml (screening:), code:python (if "volume_min" in params:), Dividend, Example Config, Extending Filters, Flags (+6 more)
...
Cohesion: 0.24
Nodes (9): cached(), _deserialize(), _make_key(), _path(), Decorator. Caches return value as JSON keyed by namespace + args., Decorator. In-process memory layer first, then file cache, then live call., _read(), _serialize() (+1 more)
...
Cohesion: 0.2
Nodes (9): Backtest Engine Implementation Plan, code:block1 (backtest/), code:python (from datetime import date), code:python (from decimal import Decimal), code:bash (git add core/context.py tests/test_context.py), code:bash (git add -A), File Structure, Task 10: BacktestContext (+1 more)
...
Cohesion: 0.67
Nodes (3): code:python (from core.context import BacktestContext), code:bash (git add strategies/basic.py), Task 12: Basic Strategy

## Knowledge Gaps
- **170 isolated node(s):** `Decorator. In-process memory layer first, then file cache, then live call.`, `Basic equal-weight strategy using live OpenBB data (yfinance/FMP).`, `Basic equal-weight strategy using mock (deterministic) data.`, `From 10000 cash, buy 2 tickers equally.`, `Each ticker gets equal share of total value.` (+165 more)
...
- **Why does `BacktestConfig` connect `Advance & Config` to `Mock FX Rates Tests`, `Mock FX Rates Provider`, `BacktestContext Methods`, `Entry Point`?**
...
- **What connects `Decorator. In-process memory layer first, then file cache, then live call.`, `Basic equal-weight strategy using live OpenBB data (yfinance/FMP).`, `Basic equal-weight strategy using mock (deterministic) data.` to the rest of the system?**
